...
<div class="app-main__inner">
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>KartuAnggota <?= ucwords(unslugify($slug)) ?>
			...
			<div class="page-title-actions">
				<nav class="" aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="active breadcrumb-item" aria-current="page">Tambah KartuAnggota</li>
					</ol>
				</nav>
			</div>
		</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header">
			<i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Tambah KartuAnggota
		</div>
		<div class="card-body">
			...
			<form id="frm_create" class="col-md-12 mx-auto" method="post" action="<?= base_url(
																						'master/kartuanggota/create?slug=' . $slug
																					) ?>">
				<div class="form-row">
					<div class="col-md-4">
						<div class="position-relative form-group">
							...
							<select class="form-control" name="category" id="category" tabindex="-1" aria-hidden="true">
								<?php foreach (get_ref('ref-kartuanggota', 'slug') as $row) : ?>
									<option value="<?= $row->slug ?>" <?= ($row->slug == $slug) ? 'selected' : '' ?>><?= $row->name ?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
					...
					<div class="col-md-4">
						<div class="position-relative form-group">
							<label for="title">Nama KartuAnggota*</label>
							<div>
								<input type="text" class="form-control" name="title" id="title" placeholder="Judul KartuAnggota " value="<?= set_value('title') ?>" />
							</div>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div>
						<textarea id="content" name="content" placeholder="Keterangan " rows="2" class="form-control autosize-input" style="min-height: 38px;"><?= set_value('content') ?></textarea>
					</div>
				</div>
