		...
		<div class="modal-content">
			...
			<div class="card-body">
				<form name="form_items" id="form_items">
					<table style="width: 100%;" id="tbl_return" class="table table-hover table-striped table-borderless" style="z-index:1000">
			...
			<div class="modal-footer">
				<div class="d-block">
					<button type="button" id="add_to_cart" class="btn btn-success" data-toggle="tooltip" data-placement="left" title="Tambah ke Troli Pengembalian"><i class="fa fa-plus"></i> Tambah ke Troli</button>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	$('#modal_loan').on('shown.bs.modal', function() {
		t_modal = $('#tbl_return').DataTable({
			"processing": true,
			"retrieve": true,
			"serverSide": true,
			"ajax": {
				"url": '<?php echo site_url('api/sirkulasi-pengembalian/loan_datatable') ?>',
			},
	...
	$(".check_return").click(function() {
		$('#tbl_return input:checkbox').not(this).prop('checked', this.checked);
	});

	$('#add_to_cart').click(function() {
		var form = $('#form_items');
		var serialize_bulk = form.serialize();
		var url = "<?= base_url('sirkulasi-pengembalian/cart_insert') ?>" + '?' + serialize_bulk;
