					...
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								...
								<div>
									<input required type="text" class="form-control" id="frm_add_Nama" name="Pekerjaan" placeholder="Jenis Pekerjaan" value="" />
								</div>
							</div>
						</div>
					</div>
...
<script>
    $('#frm_add').submit(function(event) {
		...
		var url = "<?=base_url('api/jenis-pekerjaan/create')?>";
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					title: 'Berhasil',
					html: 'Jenis Pekerjaan berhasil ditambah.',
					type: 'success',
