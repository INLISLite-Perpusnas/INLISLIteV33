        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="header-icon lnr-pencil icon-gradient bg-plum-plate"> </i> Edit Status Perkawinan
                </h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="frm_update" method="post" data-action="<?= base_url('api/master-status-perkawinan/edit') ?>" data-id="">
					...
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								...
								<div>
									<input required type="text" class="form-control" id="frm_update_Nama" name="Nama" placeholder="Status Perkawinan" value="" />
								</div>
							</div>
						</div>
					</div>
    ...
    $('#frm_update').submit(function(event) {
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					title: 'Berhasil',
					html: 'Status Perkawinan berhasil disimpan.',
					type: 'success',
