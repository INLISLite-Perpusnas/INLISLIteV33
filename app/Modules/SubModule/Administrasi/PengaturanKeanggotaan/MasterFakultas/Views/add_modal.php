					...
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								<label for="Nama">Master Fakultas</label>
								<div>
									<input required type="text" class="form-control" id="frm_add_Nama" name="Nama" placeholder="Nama Fakultas" value="" />
								</div>
							</div>
						</div>
					</div>
...
<script>
    $('#frm_add').submit(function(event) {
		...
		var url = "<?=base_url('api/fakultas/create')?>";
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					title: 'Berhasil',
					html: 'Master Fakultas berhasil ditambah.',
					type: 'success',
