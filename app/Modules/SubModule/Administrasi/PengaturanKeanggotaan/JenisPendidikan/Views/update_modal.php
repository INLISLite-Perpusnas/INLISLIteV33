        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="header-icon lnr-pencil icon-gradient bg-plum-plate"> </i> Edit Jenis Pendidikan
                </h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="frm_update" method="post" data-action="<?= base_url('api/jenis-pendidikan/edit') ?>" data-id="">
				<div class="modal-body">
                    <div id="frm_create_message"></div>
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								<label for="Nama">Jenis Identitas</label>
								<div>
									<input required type="text" class="form-control" id="frm_update_Nama" name="Nama" placeholder="Jenis Pendidikan" value="" />
								</div>
							</div>
						</div>
					</div>
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					...
					window.location.href = `<?=base_url('master-jenis-pendidikan')?>`;
				});
			} else {				
				Swal.fire({
					title: 'Oups',
