					...
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								<label for="Nama">Master Kelas</label>
								<div>
									<input required type="text" class="form-control" id="frm_update_Nama" name="namakelassiswa" placeholder="Master kelas" value="" />
								</div>
							</div>
						</div>
					</div>
	...
	$("body").on("click", ".show-data", function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#frm_update_Nama').val(response.namakelassiswa);
    ...
    $('#frm_update').submit(function(event) {
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					title: 'Berhasil',
					html: 'Kelas siswa berhasil disimpan.',
					type: 'success',
