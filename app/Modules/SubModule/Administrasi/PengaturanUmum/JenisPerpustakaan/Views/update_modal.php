        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="header-icon lnr-pencil icon-gradient bg-plum-plate"> </i> Edit Jenis Perpustakaan
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="frm_update" method="post" data-action="<?= base_url('api-jenis-perpustakaan/edit') ?>" data-id="">
				<div class="modal-body">
                    <div id="frm_create_message"></div>
					<div class="form-row">
						<div class="col-lg-12">
							<div class="form-group">
								<label for="Name">Jenis Perpustakaan</label>
								<div>
									<input required type="text" class="form-control" id="frm_update_Name" name="Name" placeholder="Jenis Perpustakaan" value="" />
								</div>
							</div>
						</div>
					</div>
    ...
    $('#frm_update').submit(function(event) {
		...
		.done(function(res) {
			...
			if (res.error == false) {
				Swal.fire({
					title: 'Berhasil',
					html: 'Jenis Perpustakaan berhasil disimpan.',
					type: 'success',
