...
<div class="app-main__inner">
            ...
            <div class="page-title-actions">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-item" aria-current="page">Ubah <?= $halaman->category ?> </li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="main-card mb-3 card">
        <div class="card-header">
            <i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Ubah Halaman
        </div>
        <div class="card-body">
                ...
                <div class="form-row">
                    <div class="col-md-12">
                        <div class="position-relative form-group">
                            <label for="name">Judul Halaman*</label>
                            <div>
                                <input type="text" class="form-control" id="title" name="title" placeholder="Judul Halaman" value="<?= set_value(
                                                                                                                                        'name',
                                                                                                                                        $halaman->title
                                                                                                                                    ) ?>" />
                                <small class="info help-block text-muted">Permalink: <a href="<?= permalink(
                                                                                                    'halaman/' . $halaman->slug
                    ...
                    <div class="col-md-4">
                        <div class="position-relative form-group">
                            ...
                            <select class="form-control" name="category" id="category" tabindex="-1" aria-hidden="true">
                                <?php foreach (get_ref('ref-halaman', 'slug') as $row) : ?>
                                    <option value="<?= $row->name ?>" <?= $row->name == $halaman->category
                                                                            ? 'selected'
                                                                            : '' ?>><?= $row->name ?></option>
                                <?php endforeach; ?>
                            </select>
