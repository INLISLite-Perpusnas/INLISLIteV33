<?php
// require_once('tcpdf_include.php'); // Make sure to adjust the path to the TCPDF library
...
foreach ($LabelData as $LabelData) {
	...
	$barcodeImage = $pdf->write1DBarcodePNG($LabelData['Barcode'], 'C128', '', '', 20, 0.4, $barcodeParams);

	// Save the barcode image to a specific path

	$barcodeImagePath = WRITEPATH . 'barcode/' . $LabelData['Barcode'] . '.png';
	file_put_contents($barcodeImagePath, $barcodeImage);

	// $pdf->write1DBarcode($LabelData['Barcode'], 'C128', '', '', '', 20, 0.4, $barcodeParams);
                ...
                <table cellpadding="0" cellspacing="0" style="width:255px;" nobr="true">
                    ...
                    <tr>
                        <td style="height:90px; width:75%; text-align: center;padding-left: 3px; padding-right: 3px;border-left:solid 1px #CCC; border-bottom:solid 1px #CCC; border-right:solid 1px #CCC;">
                            ...
                            ' . $LabelData['Title'] . '
                            <br>
                            <img src="' . $barcodeImagePath . '" width="100">
                            <br>                           
                            </span>
                            *' . $LabelData['Barcode'] . '*
                        </td>
